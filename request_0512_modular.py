...
# - IR 走本地 IIO（侧向探头装反已对调：逻辑 left=device3、right=device4；前=5 / 对应 IDX_0/IDX_2 见 _get_obstacle_status）
...
os.environ.setdefault("IR_IIO_RIGHT_DEVICE", "3")
os.environ.setdefault("IR_IIO_LEFT_DEVICE", "4")
...
class PersonTracker:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if self._vision_engine == "rknn":
            ...
            logger.info(
                "RKNN camera source: enabled=%s device=%s request=%dx%d@%.1ffps fourcc=%s",
                RKNN_CAMERA_ENABLE,
                RKNN_CAMERA_DEVICE,
                RKNN_CAMERA_WIDTH,
                RKNN_CAMERA_HEIGHT,
    ...
    def _send_robot_command(self, action: int):
        ...
        if action in (ACTION_STEER_LEFT, ACTION_STEER_RIGHT):
            steer_cap = min(MAX_FORWARD_PERCENT, STEER_PERCENT_LIMIT)
            base = max(0, min(steer_cap, int(getattr(self, "_current_steer_base_percent", 0))))
            inner_ratio = max(
                0,
            ...
            inner = max(MIN_FORWARD_PERCENT, min(steer_cap, inner))
            outer = max(MIN_FORWARD_PERCENT, min(steer_cap, outer))
            FWD = 0x01
    ...
    def _get_obstacle_status(self) -> dict:
        ...
        return {
            "front": front,  # IR1 = 前方
            # 侧向装反已对调：left←IDX_0(device3)，right←IDX_2(device4)
            "left": IR.is_triggered(IR.IDX_0),
            "right": IR.is_triggered(IR.IDX_2),
        }
    ...
    def run(self):
        ...
        timeout_sec = 0 if self._vision_engine in ("sample_reid", "rknn") else GET_FRAME_TIMEOUT
        try:
            while self.running:
                if timeout_sec > 0:
